...
def parse_commandline(args: List[str]) -> Tuple[List[str], str, str]:
    ...
    posbeds, negbeds = glob(os.path.join(posdir, "*.bed")), glob(os.path.join(negdir, "*.bed"))
    if not posbeds:
        raise FileNotFoundError(f"Positive data folder {posdir} does not contain BED files")
    if not negbeds:
        raise FileNotFoundError(f"Negative data folder {negdir} does not contain BED files")
...
def construct_dirtree(datadir: str) -> Tuple[str, str, str, str]:
    ...
    trainposdir, trainnegdir = os.path.join(traindir, "positive"), os.path.join(traindir, "negative")
    testposdir, testnegdir = os.path.join(testdir, "positive"), os.path.join(testdir, "negative")
...
def remove_duplicates(bedfile: str, fname: str, outdir: str) -> str:
    ...
    bed[seqnamescol] = bed.apply(lambda x: compute_seqname(x.iloc[0], x.iloc[1], x.iloc[2]), axis=1)
...
def split_train_test(bedfile: str, testchrom: str, fname: str, traindir: str, testdir: str) -> Tuple[str, str]:
    bed = pd.read_csv(bedfile, sep="\t", header=None)
...
def compute_background_data(trainpos: str, trainneg: str, testpos: str, testneg: str, testchrom: str, fname: str, trainnegdir: str, testnegdir: str):
    ...
    test_th = test_th if len(testneg_chrom[testchrom]) > test_th else len(testneg_chrom[testchrom])
...
def split_dataset(positive: str, negative: str, genome: str, trainposdir: str, trainnegdir: str, testposdir: str, testnegdir: str):
...
def main():
    ...
    for posbed in posbeds:
        split_dataset(posbed, negbed, genome, trainposdir, trainnegdir, testposdir, testnegdir) 
