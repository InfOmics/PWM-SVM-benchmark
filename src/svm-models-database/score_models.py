...
def parse_commandline(args: List[str]) -> Tuple[List[str], List[str], List[str]]:
    ...
    if not os.path.isdir(testposdir):
        raise FileNotFoundError(f"Unable to locate positive test data folder {testposdir}")
    if not os.path.isdir(testnegdir):
        raise FileNotFoundError(f"Unable to locate negative test data folder {testnegdir}")
...
def score(testpos: str, testneg: str, model: str, scores_fname: str) -> None:
    ...
    code = subprocess.call(f"{GKMPREDICT} -T 16 -v 0 {testpos} {model} {scorespos}", shell=True)
    if code != 0:
    ...
    code = subprocess.call(f"{GKMPREDICT} -T 16 -v 0 {testneg} {model} {scoresneg}", shell=True)
    if code != 0:
...
def score_models(testposdir: str, testnegdir: str, modelsdir: str, scoresdir: str):
    ...
    experiment_names = [os.path.basename(m).split(".")[0] for m in glob(os.path.join(modelsdir, "*.model.txt"))]
    for experiment_name in experiment_names:  # score each trained model
...
def evaluate_models_performance(scoresdir: str):
    ...
    for experiment_name in experiment_names:  # iterate over all models' scores
        ...
        scorespos = read_scores(os.path.join(scoresdir, f"{experiment_name}.scores.pos.txt"), True)
        scoresneg = read_scores(os.path.join(scoresdir, f"{experiment_name}.scores.neg.txt"), False)
...
def main():
    ...
    score_models(testposdir, testnegdir, modelsdir, scoresdir)  # score each trained model
