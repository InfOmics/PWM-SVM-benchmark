...
from sklearn.metrics import precision_score, recall_score, confusion_matrix, precision_recall_curve, roc_curve, auc, f1_score
...
COMPARISONS = ["size", "width", "optimal-global", "optimal-local", "kernel"]  # benchmark comparisons performed
...
TESTSLIST = [("shuffle", "dnase-1"), ("dnase", "dnase-1"), ("shuffle", "shuffle"), ("dnase", "shuffle"), ("shuffle", "dnase"), ("dnase", "dnase")]
...
PERFCOLS = ["EXPERIMENT", "PRECISION", "RECALL", "AUPRC", "TPR", "FPR", "AUROC", "F1"]  # performance report column names
...
def read_fasta(fasta: str) -> Dict[str, str]:
    ...
    return {seqname: sequences[i] for i, seqname in enumerate(seqnames) if len(sequences[i]) > 50}  
...
def score_pwm(fasta: str, seqname: str, sequence: str, pwm: str) -> Tuple[float, float]:
    ...
    code = subprocess.call(f"{FIMO} --oc {fasta} {FIMOOPTIONS} {pwm} {fasta}.fa", stderr=subprocess.STDOUT, stdout=subprocess.DEVNULL, shell=True)
    if code != 0:
        raise subprocess.SubprocessError(f"An error occurred while scanning sequence {fasta} with fimo")
...
def store_scores_pwm(scorespos: Dict[str, float], scoresneg: Dict[str, float], prefix: str) -> None:
    """ """
    ...
    scores = pd.concat([scorespos_df, scoresneg_df])  # concatenate the individual reports
...
def score_sequence_pwm(seqdata: Tuple[str, str], pwm: str, workingdir: str) -> Tuple[str, Tuple[float, float]]:
...
def gkmpredict(testpos: str, testneg: str, model: str, prefix: str) -> None:
    ...
    code = subprocess.call(f"gkmpredict -T 16 {testpos} {model} {scorespos}", stderr=subprocess.STDOUT, stdout=subprocess.DEVNULL, shell=True)  # score positive sequences
    if code != 0:
        raise subprocess.SubprocessError(f"An error occurred while scoring positive sequences {testpos}")
    code = subprocess.call(f"gkmpredict -T 16 {testneg} {model} {scoresneg}", stderr=subprocess.STDOUT, stdout=subprocess.DEVNULL, shell=True)  # score negative sequences
    if code != 0:
        raise subprocess.SubprocessError(f"An error occurred while scoring negative sequences {testneg}")
    store_scores_svm(scorespos, scoresneg, prefix)


def score_models_size(modelsdir: str, scoresdir: str, testdatadir_pos: str, testdatadir_neg: str, bg: str) -> None:
    """ """
    # retrieve experiment names
    experiment_names = {os.path.basename(d).split("_")[0] for d in glob(os.path.join(modelsdir, "size_500/*_streme"))}
...
def score_models_width(modelsdir: str, scoresdir: str, testdatadir_pos: str, testdatadir_neg: str, bg: str) -> None:
    """ """
    # retrieve experiment names
    experiment_names = {os.path.basename(d).split("_")[0] for d in glob(os.path.join(modelsdir, "width_50/*_streme"))}
    assert len(experiment_names) == 59  # should be 59 experiments
    for width in WIDTHS:
        ...
        for experiment_name in tqdm(experiment_names):
            ...
            testneg = os.path.join(testdatadir_neg, f"{experiment_name}_{bg}_neg_test.fa")
            try:
                fimo(testpos, testneg, os.path.join(modelsdir_width, f"{experiment_name}_meme/meme.txt"), os.path.join(scoresoutdir, f"{experiment_name}_meme"))  # meme
                fimo(testpos, testneg, os.path.join(modelsdir_width, f"{experiment_name}_streme/streme.txt"), os.path.join(scoresoutdir, f"{experiment_name}_streme"))  # streme
                gkmpredict(testpos, testneg, os.path.join(modelsdir_width, f"{experiment_name}_svm/{experiment_name}.model.txt"), os.path.join(scoresoutdir, f"{experiment_name}_svm"))  # svm
            except Exception as e:
                raise Exception(f"Scoring failed on {testpos}, {testneg}") from e
            

def score_models_global(modelsdir: str, scoresdir: str, testdatadir_pos: str, testdatadir_neg: str, bg: str) -> None:
    """ """
    # retrieve experiment names
    experiment_names = {os.path.basename(d).split("_")[0] for d in glob(os.path.join(modelsdir, "*_streme"))}
    assert len(experiment_names) == 59  # should be 59 experiments
    ...
    for experiment_name in tqdm(experiment_names):
        ...
        testneg = os.path.join(testdatadir_neg, f"{experiment_name}_{bg}_neg_test.fa")
        try:
            fimo(testpos, testneg, os.path.join(modelsdir, f"{experiment_name}_meme/meme.txt"), os.path.join(scoresdir, f"{experiment_name}_meme"))  # meme
            fimo(testpos, testneg, os.path.join(modelsdir, f"{experiment_name}_streme/streme.txt"), os.path.join(scoresdir, f"{experiment_name}_streme"))  # streme
            gkmpredict(testpos, testneg, os.path.join(modelsdir, f"{experiment_name}_svm/{experiment_name}.model.txt"), os.path.join(scoresdir, f"{experiment_name}_svm"))  # svm
        except Exception as e:
            raise Exception(f"Scoring failed on {testpos}, {testneg}") from e
...
def score_models(comparison: str, datadir: str, benchmarkdir: str):
    ...
    if comparison == COMPARISONS[0]:  # compare performance on different dataset sizes
        for bg_model, bg_score in TESTSLIST:
            modelsdir = os.path.join(benchmarkdir, f"models/dataset-size-comparison/{bg_model}")
            ...
            scoresdir = os.path.join(benchmarkdir, f"scores/dataset-size-comparison/{bg_model}-{bg_score}")
            score_models_size(modelsdir, scoresdir, testdatadir_pos, testdatadir_neg, bg_score.replace("-1", ""))
    elif comparison == COMPARISONS[1]:  # compare performance on sequence width
        for bg_model, bg_score in TESTSLIST:
            modelsdir = os.path.join(benchmarkdir, f"models/sequence-width-comparison/{bg_model}")
            testdatadir_pos = os.path.join(datadir, f"testdata/{bg_score}/positive")
            testdatadir_neg = os.path.join(datadir, f"testdata/{bg_score}/negative")
            scoresdir = os.path.join(benchmarkdir, f"scores/sequence-width-comparison/{bg_model}-{bg_score}")
            score_models_width(modelsdir, scoresdir, testdatadir_pos, testdatadir_neg, bg_score.replace("-1", ""))
        ...
        for bg_model, bg_score in TESTSLIST:
            ...
            testdatadir_pos = os.path.join(datadir, f"testdata/{bg_score}/positive")
            testdatadir_neg = os.path.join(datadir, f"testdata/{bg_score}/negative")
            scoresdir = os.path.join(benchmarkdir, f"scores/optimal-global/{bg_model}-{bg_score}")
            score_models_global(modelsdir, scoresdir, testdatadir_pos, testdatadir_neg, bg_score.replace("-1", ""))
...
def compute_f1(scores: pd.DataFrame, tool: str) -> float:
    """ """
    y_test = scores[REPORTCOLS[3]].tolist()
    y_pred_proba = scores[REPORTCOLS[1]].tolist() if tool == "svm" else scores[REPORTCOLS[2]]
...
def evaluate_models_size(scoresdir: str, perfdir: str) -> None:
    ...
    for size in SIZES:
        ...
        for tool in TOOLS:
            ...
            # retrieve experiment names
            experiment_names = {os.path.basename(d).split("_")[0] for d in glob(os.path.join(scoresdir, sizedir, f"*_{tool}.tsv"))}
            ...
            for experiment_name in tqdm(experiment_names):  # iterate over all models' scores
                scores = read_scores(os.path.join(scoresdir_size, f"{experiment_name}_{tool}.tsv"))
...
def evaluate_models_width(scoresdir: str, perfdir: str) -> None:
    ...
    for width in WIDTHS:
        ...
        for tool in TOOLS:
            # initialize models performance report
            report = {cname: [] for cname in PERFCOLS}
            experiment_names = {os.path.basename(d).split("_")[0] for d in glob(os.path.join(scoresdir, widthdir, f"*_{tool}.tsv"))}
            assert len(experiment_names) == 59
...
def evaluate_models_global(scoresdir: str, perfdir: str) -> None:
    ...
    for tool in TOOLS:
        report = {cname: [] for cname in PERFCOLS}
        experiment_names = {os.path.basename(d).split("_")[0] for d in glob(os.path.join(scoresdir, f"*_{tool}.tsv"))}
        assert len(experiment_names) == 59
        perftable = os.path.join(perfdir, f"summary_table_global_{tool}.tsv")
        for experiment_name in tqdm(experiment_names):  # iterate over all models' scores
            scores = read_scores(os.path.join(scoresdir, f"{experiment_name}_{tool}.tsv"))
            report[PERFCOLS[0]].append(experiment_name)
            report[PERFCOLS[1]].append(compute_precision(scores))  # precision
            report[PERFCOLS[2]].append(compute_recall(scores))  # recall
            report[PERFCOLS[3]].append(compute_auprc(scores))  # auprc
...
def evaluate_models_performance(comparison: str, benchmarkdir: str):
    ...
    if comparison == COMPARISONS[0]:  # compare performance on different dataset sizes
        for bg_model, bg_score in TESTSLIST:
            scoresdir = os.path.join(benchmarkdir, f"scores/dataset-size-comparison/{bg_model}-{bg_score}")
            perfdir = os.path.join(benchmarkdir, f"performance/dataset-size-comparison/{bg_model}-{bg_score}")
        ...
        for bg_model, bg_score in TESTSLIST:
            scoresdir = os.path.join(benchmarkdir, f"scores/sequence-width-comparison/{bg_model}-{bg_score}")
            perfdir = os.path.join(benchmarkdir, f"performance/sequence-width-comparison/{bg_model}-{bg_score}")
            # if not already present create performance folder
            if not os.path.isdir(perfdir): 
                os.makedirs(perfdir)  
        ...
        for bg_model, bg_score in TESTSLIST:
            scoresdir = os.path.join(benchmarkdir, f"scores/optimal-global/{bg_model}-{bg_score}")
            perfdir = os.path.join(benchmarkdir, f"performance/optimal-global/{bg_model}-{bg_score}")
