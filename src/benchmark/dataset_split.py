...
def compute_background_data(
    ...
    trainneg_fname = os.path.join(trainnegdir, f"{fname}_dnase_neg_train.bed")
    with open(trainneg_fname, mode="w") as outfile:
        outfile.write("".join(bgtrain))
    testneg_fname = os.path.join(testnegdir, f"{fname}_dnase_neg_test.bed")
...
def split_dataset(positive: str, negative: str, genome: str, traindir: str, testdir: str, shuffle: bool):
    ...
    trainposdir = os.path.join(traindir, "shuffle/positive") if shuffle else os.path.join(traindir, "dnase/positive")
    trainnegdir = os.path.join(traindir, "shuffle/negative") if shuffle else os.path.join(traindir, "dnase/negative")
    testposdir = os.path.join(testdir, "shuffle/positive") if shuffle else os.path.join(testdir, "dnase/positive")
    testnegdir = os.path.join(testdir, "shuffle/negative") if shuffle else os.path.join(testdir, "dnase/negative")
