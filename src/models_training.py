...
def meme(positive: str, outdir: str) -> str:
    outprefix = os.path.splitext(os.path.basename(positive))[0].replace("_train", "")
...
def streme(positive: str, negative: str, outdir: str) -> str:
    ...
    try:
        subprocess.call(f"streme -oc {outdir} {STREMEDEFAULT} --n {negative} --p {positive}", shell=True)
    except OSError as e:
        raise OSError("STREME training failed")
    return os.path.join(outdir, "streme.txt")
